...
export default function ContributeHero({
  ...
  return (
    <div className={`w-[100%] flex justify-center items-center px-8 py-8 sm:p-16 relative mb-[-80px] ${container}`}>
      <div className={`flex flex-col justify-center items-center text-center w-[80%] min-h-[500px]`}>
        ...
        <h1 className={`z-2 md:z-10 opacity-100 font-be-vietnam font-bold text-[1.7rem] xsm:text-[2.4rem] ${heading}`}>
          {HeroContent.heading}
        </h1>
       {HeroContent.subheading ? <h3 className={`text-[1rem] xsm:text-[1.3rem] font-be-vietnam font-bold mb-2 sm:mb-5 z-2 md:z-10 opacity-100 ${subheading} `}>
        ...
        {HeroContent.HeroLegend.map((item, index) => (
          <p
            className={`font-semibold text-[0.8rem] w-[150%] sm:text-[1.2rem] xsm:w-[120%] sm:w-[75%] font-be-vietnam mb-3 tracking-normal sm:tracking-wide z-8 md:z-10 opacity-100 ${paragraph}`}
            key={index}
          >
            {item}
          </p>
